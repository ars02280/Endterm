...
public class ProductRepository implements CrudRepository<Product, Long> {
    ...
    public Product getByName(String name) {
        String sql = "SELECT p.id, p.name, p.price, p.type, p.category_id, c.name as category_name FROM products p JOIN categories c ON p.category_id = c.id WHERE p.name = ?";
        try (Connection conn = databaseConfig.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
